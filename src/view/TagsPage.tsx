  ...
  return <>
    ...
    <section className="mt-10">
      ...
      <div className="mt-5 grid gap-3 rounded-2xl border border-neutral-800 bg-neutral-900/60 p-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-xs text-neutral-400">Direction<Select   value={String(ruleDraft.direction) || "none"} onValueChange={selected => { const value = selected === "none" ? "" : selected; setRuleDraft({ ...ruleDraft, direction: value as TagRuleDraft["direction"] }) }}><SelectTrigger aria-label="Direction" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="outflow">Outflow</SelectItem><SelectItem value="inflow">Inflow</SelectItem></SelectContent></Select></label>
        <label className="text-xs text-neutral-400">Match mode<Select   value={String(ruleDraft.matchMode) || "none"} onValueChange={selected => { const value = selected === "none" ? "" : selected; setRuleDraft({ ...ruleDraft, matchMode: value as TagRuleDraft["matchMode"] }) }}><SelectTrigger aria-label="Match mode" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent>{matchModeOptions.map(mode => <SelectItem key={mode} value={mode}>{titleCase(mode)}</SelectItem>)}</SelectContent></Select></label>
