...
function OrderDetail({ order, accounts, busy, action }: { order: AmazonOrderDetail; accounts: Account[]; busy: boolean; action: (path: string, body: unknown) => Promise<string | undefined> }) {
  ...
  return <article className="min-w-0 break-words space-y-5 rounded-xl border border-neutral-700 p-4 sm:p-6">
    ...
    {order.matchingSkipped ? <div className="space-y-2 text-sm"><p className="text-neutral-400">You marked the bank statement as unavailable. The purchase still counts in spending. Resume matching if you import the statement later.</p><Button variant="outline" disabled={busy} onClick={() => action("/matching", { orderId: order.id, skipped: false })}>Resume payment matching</Button></div> : !order.outsideTracking && order.remainingMinor !== 0 && <details className="text-sm"><summary className="cursor-pointer text-neutral-400">Skip payment matching</summary><p className="my-2 text-neutral-400">Use this when you do not have the bank statement for this purchase. It removes the order from unmatched payments, keeps its spending, and can be undone. Refunds and conflicting order details still need review.</p><Button variant="outline" disabled={busy} onClick={async () => { const failure = await action("/matching", { orderId: order.id, skipped: true }); if (!failure) { setSelected(null); setSearchOpen(false); } }}>Statement unavailable — skip matching</Button></details>}
