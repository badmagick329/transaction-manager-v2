...
export function RecurringPaymentsPage({ accounts, seedTransactionId, clearSeed, selectedPaymentId }: { selectedPaymentId: number | null; accounts: Account[]; seedTransactionId: number | null; clearSeed: () => void }) {
  ...
  function edit(value: Draft) { setPreview(null); setMethod(null); setDraft(value); setAmount((value.amountMinor / 100).toFixed(2)); }
  async function load() {
  ...
  return <section className="mt-8 space-y-6">
    ...
    {method && <form className="space-y-4 rounded-xl border border-neutral-700 p-5" onSubmit={e => { e.preventDefault(); void saveMethod(); }}>
      <h3 className="font-medium">Change matching or billing</h3>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm">Statement description or prefix<input className={field} required maxLength={500} value={method.description} onChange={e => setMethod({ ...method, description: e.target.value })} /></label>
        <label className="text-sm">Effective from<input className={field} type="date" required value={method.effectiveDate} onChange={e => setMethod({ ...method, effectiveDate: e.target.value })} /></label>
        <label className="text-sm">Description matching<select className={field} value={method.matchMode} onChange={e => setMethod({ ...method, matchMode: e.target.value as PaymentMethodChange["matchMode"] })}><option value="exact">Exact description</option><option value="starts_with">Starts with</option></select></label>
        <label className="text-sm">New billing date (optional)<input className={field} type="date" value={method.anchorDate ?? ""} onChange={e => setMethod({ ...method, anchorDate: e.target.value || null })} /></label>
        <label className="text-sm">New frequency (optional)<select className={field} value={method.frequency ?? ""} onChange={e => setMethod({ ...method, frequency: e.target.value as PaymentMethodChange["frequency"] || null })}><option value="">Keep preceding frequency</option>{frequencies.map(f => <option key={f} value={f}>{titleCase(f)}</option>)}</select></label>
        <label className="text-sm">New expected amount (optional)<input className={field} type="number" min="0.01" step="0.01" value={method.amountMinor === null ? "" : method.amountMinor / 100} onChange={e => setMethod({ ...method, amountMinor: e.target.value === "" ? null : Math.round(Number(e.target.value) * 100) })} /></label>
      </div>
      ...
      <div className="flex gap-2"><button className={button} disabled={busy}>{preview?.key === previewKey("/api/recurring-payments/method", method) ? "Confirm and save" : "Preview matching payments"}</button><button type="button" className={button} disabled={busy} onClick={() => setMethod(null)}>Cancel</button></div>
    </form>}
    ...
    {draft && <form className="space-y-4 rounded-xl border border-neutral-700 p-5" onSubmit={event => { event.preventDefault(); void save({ ...draft, amountMinor: Math.round(Number(amount) * 100) }, true); }}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="text-sm">Description matching<select className={field} value={draft.matchMode} onChange={e => setDraft({ ...draft, matchMode: e.target.value as Draft["matchMode"] })}><option value="exact">Exact description</option><option value="starts_with">Starts with</option></select></label>
        ...
        <label className="text-sm">Statement description<input className={field} required maxLength={500} value={draft.description} onChange={e => setDraft({ ...draft, description: e.target.value })} /><span className="text-xs text-neutral-400">Ignores case and extra spaces. Review the preview before saving.</span></label>
        <label className="text-sm">Expected amount<input className={field} disabled={!!draft.id} type="number" min="0.01" step="0.01" required value={amount} onChange={e => setAmount(e.target.value)} /></label>
        ...
        <label className="text-sm">Frequency<select className={field} disabled={!!draft.id} value={draft.frequency} onChange={e => setDraft({ ...draft, frequency: e.target.value as Draft["frequency"] })}>{frequencies.map(v => <option key={v} value={v}>{v === "quarterly" ? "Quarterly · every 3 months" : titleCase(v)}</option>)}</select></label>
        <label className="text-sm">Known or next billing date<input className={field} disabled={!!draft.id} type="date" required value={draft.anchorDate} onChange={e => setDraft({ ...draft, anchorDate: e.target.value })} /></label>
      ...
      {draft.id && <p className="text-xs text-neutral-400">These are the original matching settings. Use Change matching or billing for dated changes.</p>}
      ...
      <div className="flex gap-2"><button className={button} disabled={busy}>{busy ? "Working…" : preview?.key === previewKey("/api/recurring-payments", { ...draft, amountMinor: Math.round(Number(amount) * 100) }) ? "Confirm and save" : "Preview matching payments"}</button><button className={button} type="button" disabled={busy} onClick={() => { setDraft(null); clearSeed(); }}>Cancel</button></div>
    </form>}
