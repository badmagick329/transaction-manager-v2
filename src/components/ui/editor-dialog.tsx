...
export function EditorDialog({ title, children, busy, onClose }: { title: string; children: ReactNode; busy: boolean; onClose: () => void }) {
  ...
  const originCard = useRef(origin.current?.closest<HTMLElement>("article"));
  return <Dialog.Root open onOpenChange={open => { if (!open && !busy) onClose(); }}>
    <Dialog.Portal>
      <Dialog.Overlay className="fixed inset-0 z-40 bg-black/70" />
      <Dialog.Content aria-describedby={undefined} onCloseAutoFocus={event => { event.preventDefault(); (origin.current?.isConnected ? origin.current : originCard.current)?.focus({ preventScroll: true }); }} onPointerDownOutside={event => event.preventDefault()} className="fixed left-1/2 top-1/2 z-50 max-h-[90dvh] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-neutral-700 bg-neutral-950 p-6 text-neutral-100 shadow-2xl">
