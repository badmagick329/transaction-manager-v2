...
export function TagsPage({ tags, rules, accounts, loading, savingKey, ruleDraft, setRuleDraft, createTag, renameTag, deleteTag, saveRule, deleteRule }: Props) {
  const ruleEditor = useRef<HTMLDetailsElement>(null);
  useEffect(() => { if ((ruleDraft.ruleId || ruleDraft.description) && ruleEditor.current) { ruleEditor.current.open = true; ruleEditor.current.scrollIntoView({ block: "center", behavior: "smooth" }); ruleEditor.current.querySelector<HTMLElement>("button")?.focus({ preventScroll: true }); } }, [ruleDraft.ruleId]);
  ...
  return <>
    <section className="mt-8">
      ...
      <div className="mt-5 space-y-2">{tags.map(tag => <div key={tag.id} className="flex flex-col gap-2 rounded-xl border border-neutral-800 bg-neutral-900/60 p-3 sm:flex-row sm:items-center">
        <input className="min-w-0 flex-1 rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm" aria-label={`Name for ${tag.name}`} value={nameDrafts[tag.id] ?? tag.name} onChange={event => setNameDrafts(current => ({ ...current, [tag.id]: event.target.value }))} maxLength={50} />
    ...
    <section className="mt-10">
      ...
      <details ref={ruleEditor} className="mt-4"><summary className="cursor-pointer text-sm text-neutral-300">{ruleDraft.ruleId ? "Edit rule" : "Add automatic rule"}</summary><div className="mt-3 grid gap-3 rounded-2xl border border-neutral-800 bg-neutral-900/60 p-4 sm:grid-cols-2 lg:grid-cols-3">
        <label className="text-xs text-neutral-400">Tag<Select   value={String(ruleDraft.tagId) || "none"} onValueChange={selected => { const value = selected === "none" ? "" : selected; setRuleDraft({ ...ruleDraft, tagId: value }) }}><SelectTrigger aria-label="Tag" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Select tag</SelectItem>{tags.map(tag => <SelectItem key={tag.id} value={String(tag.id)}>{tag.name}</SelectItem>)}</SelectContent></Select></label>
        <label className="text-xs text-neutral-400">Provider<Select   value={String(ruleDraft.sourceId) || "none"} onValueChange={selected => { const value = selected === "none" ? "" : selected; setRuleDraft({ ...ruleDraft, sourceId: value }) }}><SelectTrigger aria-label="Provider" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">Select provider</SelectItem>{sources.map(([id, name]) => <SelectItem key={id} value={String(id)}>{name}</SelectItem>)}</SelectContent></Select></label>
        <label className="text-xs text-neutral-400">Direction<Select   value={ruleDraft.direction} onValueChange={value => { setRuleDraft({ ...ruleDraft, direction: value as TagRuleDraft["direction"] }) }}><SelectTrigger aria-label="Direction" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="outflow">Outflow</SelectItem><SelectItem value="inflow">Inflow</SelectItem></SelectContent></Select></label>
        <label className="text-xs text-neutral-400">Match mode<Select   value={ruleDraft.matchMode} onValueChange={value => { setRuleDraft({ ...ruleDraft, matchMode: value as TagRuleDraft["matchMode"] }) }}><SelectTrigger aria-label="Match mode" className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-2 py-1.5 text-sm"><SelectValue /></SelectTrigger><SelectContent>{matchModeOptions.map(mode => <SelectItem key={mode} value={mode}>{titleCase(mode)}</SelectItem>)}</SelectContent></Select></label>
