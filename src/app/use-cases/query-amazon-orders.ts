...
export function queryAmazonOrders(repository: AmazonRepository, filters: { q?: string; from?: string; to?: string; status?: string; offset?: number; limit?: number; includeOlder?: boolean }) {
  ...
  const orders = snapshot.orders.map(o => ({ ...o, spendingMinor: purchaseSpending(o.data), status: o.data.orderDate < trackingStart ? "outside-tracking" : orderStatus(o, snapshot.links), remainingMinor: remaining(o, snapshot.links, "purchase") }));
