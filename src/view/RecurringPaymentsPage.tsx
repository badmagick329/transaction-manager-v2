...
import type { SpendingControl } from "../app/recurring-payments";
...
import { frequencies, frequencyLabels, type PaymentMethodChange, type RecurringPaymentInput, type previewRecurringChange, type recurringOverview } from "../app/recurring-payments";
...
export function RecurringPaymentsPage({ accounts, seedTransactionId, clearSeed, selectedPaymentId }: { selectedPaymentId: number | null; accounts: Account[]; seedTransactionId: number | null; clearSeed: () => void }) {
  const [section, setSection] = useState<"overview" | "manage">(selectedPaymentId !== null || seedTransactionId !== null ? "manage" : "overview");
  const [focusedPaymentId, setFocusedPaymentId] = useState(selectedPaymentId);
  const [paymentView, setPaymentView] = useState("active");
  const [data, setData] = useState<Overview | null>(null);
  useEffect(() => { if (selectedPaymentId !== null) { setSection("manage"); setFocusedPaymentId(selectedPaymentId); setPaymentView("all"); setSearch(""); } }, [selectedPaymentId]);
  useEffect(() => {
    if (focusedPaymentId === null || !data || section !== "manage") return;
    const card = document.getElementById(`recurring-payment-${focusedPaymentId}`);
    card?.scrollIntoView({ behavior: "smooth", block: "center" });
    card?.focus({ preventScroll: true });
  }, [data, focusedPaymentId, paymentView, section]);
  ...
  async function decideTransaction(payment: Overview["payments"][number], transactionId: number, change: { oneOff?: boolean; priceWarningDismissed?: boolean }) {
    setBusy(true); setError(""); setNotice("");
    const previous = payment.transactionDecisions.find(d => d.transactionId === transactionId);
    try {
      const response = await fetch("/api/recurring-payments/transaction-decision", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId: payment.id, transactionId, oneOff: previous?.oneOff ?? false, priceWarningDismissed: previous?.priceWarningDismissed ?? false, ...change }) });
      if (!response.ok) throw new Error((await response.json()).error);
      await load();
      setNotice("Saved for this transaction only. Future price warnings remain enabled.");
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to save transaction decision."); }
    finally { setBusy(false); }
  }
  async function setControl(paymentId: number, control: SpendingControl) {
    setBusy(true); setError(""); setNotice("");
    try {
      const response = await fetch("/api/recurring-payments/spending-control", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ paymentId, control }) });
      if (!response.ok) throw new Error((await response.json()).error);
      await load();
      setNotice("Spending category saved.");
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to save spending category."); }
    finally { setBusy(false); }
  }
  function showPayment(paymentId: number) { setSection("manage"); setPaymentView("all"); setSearch(""); setFocusedPaymentId(paymentId); }
  ...
  return <section className="mt-8 space-y-6">
    ...
    <div className="flex gap-1 border-b border-neutral-800 pb-3" aria-label="Recurring payments views"><Button variant={section === "overview" ? "secondary" : "ghost"} aria-pressed={section === "overview"} onClick={() => setSection("overview")}>Overview</Button><Button variant={section === "manage" ? "secondary" : "ghost"} aria-pressed={section === "manage"} onClick={() => setSection("manage")}>Manage payments</Button></div>
    {section === "overview" && (data ? <RecurringDashboard data={data} busy={busy} onControl={setControl} onManage={showPayment} onBilling={p => startMethod({ paymentId: p.id, accountId: p.currentMethod.accountId, description: p.currentMethod.description, matchMode: p.currentMethod.matchMode, frequency: null, anchorDate: null, amountMinor: p.currentMethod.amountMinor, effectiveDate: new Date().toISOString().slice(0, 10) }, p.currencyCode)} /> : <p className="text-neutral-400">Loading recurring overview...</p>)}
    <div hidden={section !== "manage"}><RecurringAgentReview accounts={accounts} refreshKey={data} onChanged={load} /></div>
    ...
    {section === "manage" && [...totals].map(([currency, total]) => <p key={currency} className="text-lg">{formatMoney(total, currency)} <span className="text-sm text-neutral-400">expected monthly equivalent · active payments</span></p>)}
    ...
    <div hidden={section !== "manage"} className="space-y-6"><div className="flex flex-wrap items-center gap-2"><Input aria-label="Search recurring payments" placeholder="Search payments…" className="sm:max-w-xs" value={search} onChange={e => setSearch(e.target.value)} />{["active", "inactive", "all", "suggestions"].map(view => <Button key={view} variant={paymentView === view ? "secondary" : "ghost"} aria-pressed={paymentView === view} onClick={() => setPaymentView(view)}>{titleCase(view)}{view === "suggestions" && data ? ` (${data.suggestions.length})` : ""}</Button>)}</div>
    {!data ? <p className="text-neutral-400">Loading recurring payments…</p> : <>
      <div className={paymentView === "suggestions" ? "hidden" : "space-y-3"}><h3 className="font-medium">Tracked payments</h3>{visiblePayments.length === 0 && <p className="text-sm text-neutral-400">No payments in this view. Try another view or search.</p>}
        {visiblePayments.map(p => <article key={p.id} id={`recurring-payment-${p.id}`} tabIndex={-1} className="rounded-xl border border-neutral-800 p-4 focus:outline focus:outline-2 focus:outline-sky-500"><div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="font-medium">{p.name}</h4><p className="text-sm text-neutral-400">{titleCase(p.kind)} · {frequencyLabels[p.currentMethod.frequency]} · {p.status} · {accounts.find(a => a.id === p.currentMethod.accountId)?.name}</p><p className="mt-2 text-sm">Expected {formatMoney(p.currentMethod.amountMinor, p.currencyCode)} · {p.nextDate ? `Next expected: ${p.nextDate}` : "Schedule paused"}</p><p className="text-sm text-neutral-400">Last payment: {p.transactions.at(-1) ? `${p.transactions.at(-1)!.transactionDate.slice(0, 10)} · ${formatMoney(Math.abs(p.transactions.at(-1)!.amountMinor), p.currencyCode)}` : "No matching payment"}</p></div><div className="flex shrink-0 flex-wrap gap-2 sm:ml-auto"><Button variant="outline" disabled={busy} onClick={() => { clearSeed(); edit(p); }}>Edit details</Button><Button variant="outline" disabled={busy} onClick={() => startMethod({ matchMode: p.currentMethod.matchMode, anchorDate: null, frequency: p.currentMethod.frequency, amountMinor: p.currentMethod.amountMinor, paymentId: p.id, accountId: p.currentMethod.accountId, description: p.currentMethod.description, effectiveDate: new Date().toISOString().slice(0, 10) }, p.currencyCode)}>Update billing</Button></div></div>
          ...
          <details className="mt-3 text-sm"><summary className="cursor-pointer text-neutral-400">Payment method history</summary><p className="mt-1">Initially: {accounts.find(a => a.id === p.accountId)?.name} · {p.description}</p>{p.methods.map(m => <p key={m.effectiveDate} className="mt-1">From {m.effectiveDate}: {accounts.find(a => a.id === m.accountId)?.name} · {m.matchMode === "contains" ? "Contains: " : m.matchMode === "starts_with" ? "Starts with: " : "Exact: "}{m.description}{m.anchorDate ? ` · Billing date: ${m.anchorDate}` : ""}{m.frequency ? ` · ${frequencyLabels[m.frequency]}` : ""}{m.amountMinor !== null ? ` · ${formatMoney(m.amountMinor, p.currencyCode)}` : ""} <Button variant="outline" className="text-sky-400 hover:underline" disabled={busy} onClick={() => startMethod({ ...m, previousEffectiveDate: m.effectiveDate }, p.currencyCode)}>Correct</Button></p>)}</details><details className="mt-3 text-sm"><summary className="cursor-pointer text-neutral-400">Matching payments ({p.transactions.length})</summary><p className="mt-2 text-xs text-neutral-400">One-off adjustments stay in spending history but do not advance the billing schedule or trigger price warnings.</p>{p.transactions.map(t => {
            const decision = p.transactionDecisions.find(d => d.transactionId === t.id);
            return <div key={t.id} className="mt-3 space-y-2 border-t border-neutral-800 pt-3"><p>{t.transactionDate.slice(0, 10)} · {accounts.find(a => a.id === t.accountId)?.name} · {t.description} · {formatMoney(Math.abs(t.amountMinor), t.currencyCode)}</p><div className="flex flex-wrap items-center gap-2">{decision?.oneOff && <span className="text-sky-300">One-off adjustment</span>}{decision?.priceWarningDismissed && <span className="text-neutral-400">Price warning dismissed for this charge</span>}<Button variant="outline" size="sm" disabled={busy} onClick={() => void decideTransaction(p, t.id, { oneOff: !decision?.oneOff })}>{decision?.oneOff ? "Treat as regular payment" : "Mark as one-off adjustment"}</Button>{decision?.priceWarningDismissed && <Button variant="outline" size="sm" disabled={busy} onClick={() => void decideTransaction(p, t.id, { priceWarningDismissed: false })}>Restore price warning</Button>}</div></div>;
          })}</details>
        </article>)}
      </div>
