...
function ActionEditor({ action, accounts, payments, transactions, onChange }: { action: ReviewAction; accounts: Account[]; payments: RecurringPayment[]; transactions: RecurringTransaction[]; onChange: (action: ReviewAction) => void }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">
    ...
    {action.type !== "update" && <>
      ...
      <Choice label="Frequency" value={input.frequency ?? ""} options={[...(method ? [["", "Inherit preceding frequency"] as [string, string]] : []), ...["weekly", "monthly", "quarterly", "annual"].map(v => [v, titleCase(v)] as [string, string])]} onChange={frequency => change({ frequency: frequency || null })} />
    ...
    <Choice label="Matching" value={input.matchMode} options={[["exact", "Exact description"], ["starts_with", "Starts with"]]} onChange={matchMode => change({ matchMode })} />
