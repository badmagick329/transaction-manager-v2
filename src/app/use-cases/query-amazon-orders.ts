...
export function queryAmazonOrders(repository: AmazonRepository, filters: { q?: string; from?: string; to?: string; status?: string; offset?: number; limit?: number }) {
  ...
  const orders = snapshot.orders.map(o => ({ ...o, spendingMinor: purchaseSpending(o.data), status: orderStatus(o, snapshot.links), remainingMinor: remaining(o, snapshot.links, "purchase") }));
  ...
  return { spending, total: filtered.length, orders: filtered.slice(offset, offset + (filters.limit ?? 30)) };
}
