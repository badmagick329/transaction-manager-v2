...
function OrderDetail({ order, accounts, busy, action }: { order: AmazonOrderDetail; accounts: Account[]; busy: boolean; action: (path: string, body: unknown) => Promise<string | undefined> }) {
  ...
  return <article className="min-w-0 break-words space-y-5 rounded-xl border border-neutral-700 p-4 sm:p-6">
    ...
    {order.remainingMinor === undefined && <div className="space-y-2 rounded border border-amber-800 p-3 text-sm"><p>First, enter how much Amazon balance you used in How you paid. Then we can match the remaining card payment.</p><Button variant="outline" onClick={() => document.getElementById("amazon-payment-heading")?.scrollIntoView({ block: "start", behavior: "smooth" })}>Go to payment details</Button></div>}

    {!order.candidates.length && order.remainingMinor !== undefined && order.remainingMinor !== 0 && <p className="text-sm text-neutral-400">No suggested bank payment. You can search your transactions below.</p>}
